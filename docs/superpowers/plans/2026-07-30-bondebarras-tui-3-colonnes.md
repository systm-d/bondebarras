...
- **Budget chiffré et repli (2026-09-11, contrôleur).** L'étiquette la plus longue est `sha256:1d7018e56… (attestation orpheline)` (41 caractères, `scan.rs:362-363`) ; à 80 colonnes la colonne des ressources a 40 cellules intérieures, et `ROW_BESIDE_LABEL` (`views/repo.rs:42`) en prend aujourd'hui 33. Les 12 caractères de libellé minimum restent la cible de **chaque** ligne à 80 colonnes, obtenus en compactant la taille et le champ de fin. Pour le suffixe de classe d'une version de paquet, le repli est décidé : quand le suffixe complet ne tient pas sur la ligne à la largeur courante, il passe sur une seconde ligne du même élément, sous le libellé, **jamais raccourci** ; l'élément reste un seul élément de liste (sélection, curseur et coche portent sur les deux lignes). `a_package_row_survives_at_eighty_columns` affirme alors les deux suffixes visibles dans le `Rect` réel à 80 colonnes, sur la ligne ou sur sa seconde ligne.
- **Réponses au NEEDS_CONTEXT (2026-09-11, contrôleur).** (1) **Coche de dépôt.** La v0.5 n'archive qu'un dépôt à la fois (`selected_repo` en tient un ; `clean::execute` archive `plan.owner/plan.repo`) : ce plan ne crée pas d'archivage multiple. La coche unique survit aux déplacements du curseur et à l'arrivée des listes (depuis la Task 5 elle se perdait à chaque pause) ; un changement d'organisation l'efface toujours (exigence de la revue finale v0.5) ; `d` depuis la colonne des dépôts archive le dépôt coché même si le curseur est sur un autre — la modale de Tier 2 le nomme. (2) **Budget de ligne.** Le marqueur est collé à la case (`[x]⛑`, `[ ]•`, `[ ] `). La taille d'une ligne de ressource passe à un format compact de 5 cellules au plus, unités françaises sans espace (`467Mo`, `1.1Go`, `12Go`), réservé aux lignes de la colonne des ressources — en-têtes, jauges et colonnes 1 et 2 gardent `human_size`, et la doc du format compact dit pourquoi. Le champ de fin garde 10 cellules (`par défaut`, `PR#n ⚑`). La garantie des 12 caractères de libellé porte sur la **largeur intérieure de la colonne des ressources** : dès 40 cellules intérieures (le cas à 80 colonnes de terminal), chaque ligne garde au moins 12 caractères ; en dessous (38–39 cellules à 100–101 colonnes de terminal), elle n'est pas promise. Aucun seuil de disposition ne change.
...
- [ ] **Step 3 quater:** ce que les Tasks 5 à 7 ont ajouté et que la documentation ignore encore — dans `CLAUDE.md`, le canal de purge étiqueté (`spawn_purge` étiquette chaque message de progression avec le dépôt du plan, pour que la fin d'une purge oublie le bon cache sans toucher `clean.rs`) et la barre de progression (`views/progress.rs`) ; dans `site/`, toute maquette ou capture textuelle du pied qui montre encore `A tout ⚑` ou un pied sans les déplacements suit le pied réel (`[A] sûrs`, `[V] +à vérifier`, touches de déplacement). Vérifier par une recherche de `tout ⚑` et `select_all_stale` dans tout le dépôt, hors `target/`.
