...
- it does **not** build the release binary; `release.yml` does, as
  `cargo build --release --locked`. Keep `cargo build --release` in the
  local gate anyway: a broken release build is not something to discover at
  tag time.
